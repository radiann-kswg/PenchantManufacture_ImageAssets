...
- `m` は全334グリフ中の**最大幅**（w/ψ/φ/ω/# と同値）＝ em 幅。`I` はその **約49%** ＝ ほぼ半角。
...
> `extract_glyphs.compute_vertical_fit` で全グリフのベースラインを 67.9px 下げて解決。
> 副作用として固定縦バンドが +15.5% 広がり、既存グリフは従来比 86.6% で描画される
> （[AGENTS.md] 「縦方向の配置」参照）。
...
| P8 | B10 製図・GD&T / B12 ローマ数字（**v4.0-beta で済**。合成13〜39含む） / B15 論理・証明（**v3.4 で済**。B1 残りと同時実装が実現） | 作字 | 作者作字（残りは B10） |
