...
- `m` は全334グリフ中の**最大幅**（w/ψ/φ/ω/# と同値）＝ em 幅。`I` はその **48.6%** ＝ ほぼ半角。
...
> ビルドごとに機械判定する。v3.3-beta 時点では `А В Е О Т с о` の 7 字がラテンへ
> 統合され、逆に `К М Н х` はキリル側が正規（ギリシャ `Κ Μ Η`・ラテン `x` を吸収）。
> v3.2 まで統合されていた `С`(→C) `Х`(→Χ) 等はサブピクセル差で独立に戻った
> （[AGENTS.md] 重複排除の方針を参照）。トークン（`yua` など）とリテラル字（`А`）は
> 正規側の検索エイリアスに残るため、型番検索での引きやすさは失われない。
> v3.3-beta で実際に絵文字として増えているのは **59字**。
>
