...
### A1 上付き（0–9・演算子・n/i）
既存 0–9 と `+ - = ( )` を縮小・上寄せ配置して生成。指数・単位（m²、xⁿ、mc²）に必須。
...
### A2 下付き（0–9・演算子・n）
添字（H₂O、x₁、CO₂）に必須。既存グリフの縮小・下寄せ配置で生成。
...
### スペーサ・単位/型番合字（合成）
コードポイントを持たない独自アセット。トークンのみ定義（Unicode 非対応 = 名前・エイリアス運用）。

| 種別 | トークン | 既定名(墨) | 内容 | 主なエイリアス |
| --- | --- | --- | --- | --- |
| スペーサ | `spc` | `spcp` | 透過・固定幅の余白（薄枠オプション） | space, blank, gap |
