...
> 最終更新: 2026-09-10（v4.0-release へ追随。字形改善のみでグリフ数・統合結果は v4.0-beta と同一）
...
（v4.0-release。アクセント付き56字は v3.2-beta で実アウトラインが与えられ、cmap 統合は 0 件）。
...
合計 5 × 396 × 2 × 2 = **7,920 PNG**（v4.0-release）。サフィックス `_512` / `_128` は**高さ** px
