...
## 収録グリフ（PenchantManufacture v3.1-release）

`_original-fonts/penchant-manufacture_v3.1-release/収録グリフ.txt` に記載の作者公式グリフ:
...
cmap は 309 コードポイントだが、**同一グリフ（同一アウトライン）へ再マップされた
...
1. **cmap 再マップの統合（`extract_glyphs.py`）** — アクセント付きラテン 56 字
   ...
   グリフ名でユニーク化し、**正規グリフ 247 字のみ** SVG 化する。統合した異体字は
...
2. **描画一致の統合（`generate_decal.py` → `dedupe_renders`）** — フォント上は別グリフ
   ...
   **17 字が 12 グループへ統合され、絵文字は正味 230 字**となる。全スキーム×全サイズで
   バイト完全一致する場合のみ統合する安全側の判定。統合表は
   `docs/glyph_render_merges.json`。**ソース SVG（`src/glyphs/`）は 247 字を温存**し、
...
| 段階                       | 対象         | v3.0-release | v3.1-release |
| -------------------------- | ------------ | ------------ | ------------ |
| cmap マッピング            | —            | 210          | 309          |
| 実グリフ                   | —            | 155          | 253          |
| SVG ソース（cmap再マップ統合後） | src/glyphs   | **148**      | **247**      |
| 絵文字 PNG（描画一致統合後）     | dist/glyphs_decal | **143** | **230**      |

| 項目                 | 値                                        |
| -------------------- | ----------------------------------------- |
| フォント名           | PenchantManufacture (Regular)             |
| バージョン           | Version 1.007 (Fontself Maker 3.6.12)     |
| Units per em         | 1000                                      |
| cmap マッピング数    | 309                                       |
| 実グリフ数           | 253                                       |
...
├── src/
│   └── glyphs/                 ← グリフ SVGソース 247字（アウトライン化済み、extract_glyphs.py 出力）
├── dist/
...
PenchantManufacture.otf
  ...
  │         ├─ src/glyphs/char_*.svg  （正規247字、fontToolsのSVGPathPenで純粋パス）
...
- **Misskey**: `_exported-dist/penchant-misskey-*.zip` を管理画面から一括インポート。
  ...
  収録点数は **230字 × 5バリアント ＋ スペーサ2 = 1152点**（v3.1-release ビルド時）。
