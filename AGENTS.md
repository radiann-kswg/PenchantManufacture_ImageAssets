...
5. **半角系／全角系の分業** — 本家はプロポーショナル半角系。全角メトリクスが
   前提の和文（括弧・約物・かな）は **PenchantManufacture-CJK**（等幅・全角）で扱い、
   本家には収録しない。CJK 版も **CC BY 4.0**（2026-09-08 に有料頒布計画を撤回し
   本家と統一。ターミナルアプリ等へのソフトウェア埋め込みを想定用途とするため）。
...
8. **1 行スコープ** — 収録対象は **1 行で完結する技術表記**（型番・仕様・単位・図面注記・
   化学反応式・1 行に収まる数式）まで。分数の 2 行組版・大型演算子の上下引数・行列式・
   可変高括弧は **下流プロダクト `PenchantManufacture_ImagePipeline` の責務**であり、
   本リポジトリでは作字も合成もしない（下記「下流プロダクト」節、
   [docs/GLYPH_EXTENSION_PLAN.md] §0）。

---

## 下流プロダクト `PenchantManufacture_ImagePipeline`

多段数式・分数・行列式のような**複数行にまたがる組版**は、カスタム絵文字では実現できない
（MFM に行間・列揃えの制御機構が無く、ブラウザが CSS 高さで任意倍率に縮小描画するため
列が揃わない）。これらは本リポジトリを **git サブモジュール**として持つ別プロダクト
`PenchantManufacture_ImagePipeline` が、Bot／アプリ向けの**画像生成**として担う。

| 項目 | 内容 |
| --- | --- |
| 責務 | 分数・大型演算子の上下引数・行列式・多段数式・可変高括弧の**画像組版** |
| サブモジュール名 | 本リポジトリは ImagePipeline 側に **`basis/`** として取り込まれる |
| 入力 | **`src/glyphs/` の SVG ＋ フォントの実メトリクス**（`assets/fonts/PenchantManufacture.otf`） |
| 使わないもの | `dist/glyphs_decal*/` のデカール PNG。ハロー 10px ＋ キーライン 6px が字間に二重に入り、密な数式で字がくっつくため |
| 既存資産の再利用 | `scripts/generate_roman.py` が既に「フォントの GPOS カーニングを読んで合成 SVG を組む」実装を持つ。多段組版はその自然な拡張として実装する |

**本リポジトリ側の帰結**

- 絵文字 PNG の**幅をグリッドへ量子化しない**。ImagePipeline は SVG を直接読むため受益者が
  居らず、量子化は全点再生成と Misskey 全点再インポートのコストだけが残る
  （[GLYPH_EXTENSION_PLAN.md] §0-2）。
- 可変高括弧の上端／下端／延長片は「文字ではなく図形」であり、作字コンセプト原則 2 に反する。
  **本家では作字しない。**
- サブモジュール参照は ImagePipeline 側から張る。本リポジトリは下流を知らずにビルドできる
  状態を保つこと（`scripts/` に ImagePipeline 向けの分岐を入れない）。

---

## カスタム絵文字 点数の上限

**Misskey 3500 点**を上限とする（ピッカーの一覧性とサーバーストレージのため。
zip 一括インポートなので登録の手間は点数に比例しない）。

| 時点 | 点数 | 内訳 |
| --- | --- | --- |
| v4.0-release（現行） | **2252** | 396字 × 5 ＋ 合成ローマ数字 54 × 5 ＋ スペーサ 2（v4.0-beta から点数不変） |
| 実装予定分を加算 | **2482** | ＋ 46字 × 5（[GLYPH_EXTENSION_PLAN.md] §6-2） |
| 凍結枠を将来解凍しても | 2727 | ＋ C2 15字 / P2c 14字 / P7b 20字 × 5 |

> 実装予定 46 字のうち `⌀`(`dia`) は `Ø`(`uosk`) と描画一致統合される可能性があり、
> 統合された場合は 45 字 / 225 点 / **2477 点**になる（`dedupe_renders` がビルド時に機械判定）。

> **TODO（未定義）**: **Discord へ登録するサブセットの選定基準が、どの docs にも書かれていない。**
> Discord は 1 サーバー 50〜250 枠のため既定（墨）396 字ですら全点は入らない。
> 現状は複数サーバー分割・ブースト段階での運用が前提だが基準が明文化されていないため、
> Discord 対応の要件定義を行う際に [docs/EMOJI_TECHCODE_SPEC.md] §3 へ追記すること。

---

## 収録グリフ（PenchantManufacture v4.0-release）

`_original-fonts/penchant-manufacture_v4.0-release/収録グリフ.txt` に記載の作者公式グリフ
（v4.0-beta から収録字の増減なし）:
...
v4.0-release（2026-09-10）で **`w` `ω` とローマ数字 `Ⅸ Ⅹ Ⅺ Ⅻ` / `ⅸ ⅹ ⅺ ⅻ` の
字形が変更・改善**された。収録字（cmap 415 / 実グリフ 416）・トークン・win 帯・
描画一致統合（9 グループ）はすべて beta と同一で、絵文字点数 2252 も不変。
`name` テーブルのバージョン文字列は **Version 1.010 のまま据え置き**（beta と同値）
なので、版の識別は `_original-fonts/` のフォルダ名と README.txt の更新履歴で行う。

### ローマ数字対応（v4.0）— 専用ビルド generate_roman.py

v4.0 のローマ数字は、通常グリフと**コンセプトが異なる**:
...
- 帯を逸脱するインクは `extract_glyphs.py` が **WARN** で検出する（現行 v4.0-release は逸脱ゼロ）。
...
cmap は 415 コードポイント（v4.0-release）だが、フォント上は別グリフでも**絵文字サイズで描画が一致する字**
...
1. **cmap 再マップの統合（`extract_glyphs.py`）** — 異なるコードポイントが同一グリフを
   ...
   **v3.2-beta で全 56 字に実アウトラインが与えられ、統合件数は 0 になった**。
   現在の正規グリフは **409 字**（v4.0-release。機構自体は将来の再マップに備えて残す）。
2. **描画一致の統合（`generate_decal.py` → `dedupe_renders`）** — フォント上は別グリフ
   でも絵文字サイズで描画が完全一致する字は、出力 PNG を正規側 1 枚に統合する。
   **13 字が 9 グループへ統合され、絵文字は正味 396 字**となる。全スキーム×全サイズで
   バイト完全一致する場合のみ統合する安全側の判定。統合表は
   `docs/glyph_render_merges.json`。**ソース SVG（`src/glyphs/`）は 409 字を温存**し、
...
| 段階                       | 対象         | v3.2-beta | v3.3-beta | v3.4-release | v4.0-beta / release |
| -------------------------- | ------------ | --------- | --------- | ------------ | --------- |
...
| バージョン文字列     | Version 1.010 (Fontself Maker 3.6.12) ※beta から据え置き |
| Units per em         | 1000                                      |
...
│   ├── build_previews.py       ← README掲載画像生成 → docs/previews/{hero,glyphset}.png
│   └── build.py                ← 全ステップ一括ビルド
├── docs/
...
│   ├── GLYPH_EXTENSION_PLAN.md  ← 追加グリフ計画（§0 スコープ／§6 実装フェーズが正）
...
PenchantManufacture.otf
  ...
  │         ├─ src/glyphs/char_*.svg  （正規409字、fontToolsのSVGPathPenで純粋パス）
...
python scripts/build_previews.py         # README掲載画像 → docs/previews/hero.png, glyphset.png
...
- **Misskey**: `_exported-dist/penchant-misskey-*.zip` を管理画面から一括インポート。
  ...
  = 2252点**（v4.0-release ビルド時。v4.0-beta と同数）。
