...
## 収録グリフ（PenchantManufacture v3.3-beta）

`_original-fonts/penchant-manufacture_v3.3-beta/収録グリフ.txt` に記載の作者公式グリフ:
...
**v3.3-beta で縦バンドが `−198.2 … 793.0`（991.2 / 1000 upm）へ 0.5 unit 拡大した。**
改善された上付き数字（`² ³ ¹` 等）と `√ ∛ ∜` のインク上端が 793.0 に達し、
従来の天井 792.5 をわずかに超えたため。総高 991.2 は em(1000) 未満なので
**再スケールは発生せず**、`compute_vertical_fit` によるベースラインの下げ量が
0.5 unit（512px 出力で約 0.26px、128px で約 0.06px）増えただけである。
この結果 **全グリフの PNG がバイトレベルでは変化**するが、見た目はサブピクセル
オフセットのみで実質同一。Misskey は zip 再インポートで全点置換、Discord の
登録済み絵文字は**再アップロード不要**（視覚差なし。字形が実際に変わった
`i j r s t ν` 等 33 字と `ß` 同様の差し替えを望む場合のみ個別対応）。

> **フォント更新時に必ず確認すること**
> 新しいグリフのインクが現行バンド `−198.2 … 793.0` を逸脱すると
> `compute_vertical_fit` のベースライン位置が全グリフ一律で動く。逸脱が小さければ
> サブピクセルシフトで済む（v3.3 の +0.5 unit）が、**字面総高が em(1000) を超えると
> 全グリフが再スケールされ、Discord 登録済み絵文字の全点手動再アップロード**が
> 発生する（v3.1 で実際に起きた）。逸脱チェックは
...
2. **描画一致の統合（`generate_decal.py` → `dedupe_renders`）** — フォント上は別グリフ
   ...
   **14 字が 11 グループへ統合され、絵文字は正味 320 字**となる。全スキーム×全サイズで
   ...
   v3.3-beta のベースライン 0.5 unit シフトでアンチエイリアスの丸めが変わり、
   統合対象が v3.2 から入れ替わった: `A<=Α` `B<=Β` `C<=С` `Χ<=Х` の 4 組が
   バイト不一致となり独立し、`х<=x` が新たに一致した（見た目の同形性は不変。
   バイト一致のみを統合する安全側判定の想定内の揺れ）。
...
| 段階                       | 対象         | v3.1-release | v3.2-beta | v3.3-beta |
| -------------------------- | ------------ | ------------ | --------- | --------- |
| cmap マッピング            | —            | 309          | 327       | 340       |
| 実グリフ                   | —            | 253          | 328       | 341       |
| SVG ソース（cmap再マップ統合後） | src/glyphs   | **247**      | **321**   | **334**   |
| 絵文字 PNG（描画一致統合後）     | dist/glyphs_decal | **230** | **304**   | **320**   |

| 項目                 | 値                                        |
| -------------------- | ----------------------------------------- |
| フォント名           | PenchantManufacture (Regular)             |
| リリース             | v3.3-beta (2026-08-15)                    |
| バージョン文字列     | Version 1.008 (Fontself Maker 3.6.12)     |
| Units per em         | 1000                                      |
...
> v3.2-beta で懸案だった `name` テーブルの版数据え置きは v3.3-beta で
> **1.008 に繰り上げ済み**（`head.fontRevision` は 1.0 のままだが、
...
- **Misskey**: `_exported-dist/penchant-misskey-*.zip` を管理画面から一括インポート。
  ...
  収録点数は **320字 × 5バリアント ＋ スペーサ2 = 1602点**（v3.3-beta ビルド時）。
  字種サブカテゴリに v3.2-beta で **ラテン拡張大文字 / ラテン拡張小文字**、
  v3.3-beta で **数式記号**（科学・数式13字＋`×` `÷`）を追加。
